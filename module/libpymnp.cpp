...
extern "C"
{
    ...
    extern double mnp_deep_contig_w(int64_t n, double* contiguous_s_weights_in, double* m_weights_in, int64_t* layer_sizes_in, int64_t n_layers, double* p_y)
    {
        ...
        //Rectify with a sigmoid
        deep.rectify = [](double x){ return 2.0 * (1.0 / (1.0 + exp(-x)) ) - 1.0; };
    ...
    extern double dsf_eval(int64_t n, double* contiguous_s_weights_in, double* m_weights_in, int64_t* layer_sizes_in, int64_t n_layers, bool* p_A)
    {
        //Create Deep Submodular Function.
        std::vector<int64_t> layer_sizes(n_layers);
        for(int64_t i = 0; i < n_layers; i++) {
        ...
        return problem.eval(A);
    }

    extern double watts_strogatz_graph_cut_yprime(int64_t n, int64_t k, double beta, double* p_y)
    {
        MinCut<double> problem(n);
        problem.WattsStrogatz(k, beta);
