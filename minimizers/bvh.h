...
//TODO: pass in preallocated space
template<class DT>
        ...
        Matrix<DT>& S, Matrix<DT>& D, DT tolerance)
{
    ...
    while(1) {
        ...
        //Compute g
        Vector<DT> g(S.width() - 1);
        D.transposed().mvm(-2.0, x, 0.0, g);

        if(g.abs_max() < tolerance) { 
            std::cout << g.abs_max() << std::endl;
            return;  //Or should it be 0.0 rather than tolerance?
        }

        //y = [w(0), ..., w_k-2)
        Vector<DT> y(S.width()-1);
        y.copy(w_linear);
        w.print("w");
        y.print("y");
        ...
        //h = y + C g
        //For now we choose C = I.
        Vector<DT> h(y.length());
        h.copy(y);
        h.axpy(1.0, g);
        g.print("g");
        h.print("h");
        ...
        u_linear.copy(h);
        u(u.length()-1) = 1.0 - h.sum();

        //Compute lambda
        //Need to pick best lambda that gives us a valid v.
        //TODO: I think we need to modify the code somewhere around here to ensure that we end up with a v with nonnegative components
        w.print("w");
        u.print("u");
        DT lambda = (w(0) - u(0)) / w(0);
        for(int64_t i = 1; i < w.length()-1; i++) {
            lambda = std::max(lambda, (w(i) - u(i)) / w(i));
            std::cout << "lambda i " << (lambda, (w(i) - u(i)) / w(i)) << std::endl;
        }
        lambda = 1.0 / lambda;
        std::cout << "lambda ends up being " << lambda << std::endl;
        if(!(lambda <= 1.0 && lambda > 0.0)) std:: cout << "LAMBDA IS NOT BETWEEN 0 AND 1.0" << std::endl;
        lambda = std::max(std::min(0.0,lambda), 1.0);
        Vector<DT> v(S.width());
        v.copy(w);
        ...
        assert(v.min() > -tolerance);
        assert(u.min() > -tolerance);
        ...
        assert(Dw.dot(Dw) + Dv.dot(Dv) > 2*Dw.dot(Dv));
        ...
        if(alpha < 1.0) {
            ...
            std::cout << "Alpha is less than one" << std::endl;
            if(std::abs(alpha) <= tolerance){
                std::cout << "Alpha is zero!!" << std::endl;
                S.mvm(1.0, w, 0.0, x);
                return;
            }
        } else {
            ...
            std::cout << "Removing " << to_remove.size() << " columns " << std::endl;
...
std::vector<bool> bvh(SubmodularFunction<DT>& F, Vector<DT>& wA, DT eps, DT tolerance) 
{
    ...
    while(1) {
        ...
        w.enlarge(1);
        w(w.length()-1) = gamma;
        std::cout << "Sum of w is " << w.sum() << std::endl;
