...
void benchmark_log_det()
{
    ...
    for(int64_t i = start; i <= end; i += inc) {
        ...
        for(int64_t r = 0; r < n_reps; r++) {
            ...
            cycles_count_start();
            mnp.minimize(problem, 1e-10, 1e-5, false, &log);
...
//    std::cout << std::setw(2*fw) <<  "BLAS_s_brsmnp %";
    ...
    for(int64_t i = start; i <= end; i += inc) {
        ...
        for(int64_t r = 0; r < n_reps; r++) {
            ...
            cycles_count_start();
            auto A_brsmnp = brsmnp.minimize(problem, 1e-10, 1e-5, true, &brsmnp_log);
            cycles = (double) cycles_count_stop().cycles;
            cpu_cycles_mnp.push_back(cycles_count_stop().time);
            double brsmnp_time = cycles_count_stop().time;
            major_cycles_mnp.push_back(brsmnp_log.get_count("MAJOR TIME"));
            minor_cycles_mnp.push_back(brsmnp_log.get_count("MINOR TIME"));
